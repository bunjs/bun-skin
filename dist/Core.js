...
const _require = require('./config'),
      globalPath = _require.globalPath,
      viewExt = _require.viewExt,
      defaultPort = _require.defaultPort;
...
class Bun extends Koa {
  ...
  constructor(name, options) {
    super();
    this.name = name; // 保证 context.app 是 bun 而不是 koa

    this.context.app = this; // 获取根目录

    const rootPath = options.ROOT_PATH || path.dirname(require.main.filename); // 设置全局路径

    Object.assign(this, globalPath(rootPath)); // 初始化日志
    ...
    this.plugins = {};
    this.lib = {};
    this.app = {}; //解决后端渲染前端资源时遇到css相关文件报错的问题
  ...
  initAllApps() {
    ...
    files.forEach(filename => {
      ...
      if (stat.isDirectory()) {
        ...
        routes = new this.Routes(filename); // 将每个app相关属性存入全局备用
  ...
  run(port = defaultPort) {
    this.listen(port); // 冻结bun对象，只读，防止用户错误覆盖
